...
const ProductItem: React.FC<iProductItem & { goDetail: () => void, unLike?: (s: string) => void }> = props => {
  const { name, image, price, rating, sold, goDetail, unLike } = props;
  const w = width / 2 - 24 > 200 ? 200 : width / 2 - 24;
  const [isLike, setIsLike] = useState(unLike ? true : false);
  return (
    <TouchableOpacity
